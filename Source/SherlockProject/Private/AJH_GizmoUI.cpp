...
void UAJH_GizmoUI::OnMyEdit_Location()
{
	if ( player )
	{
		Location_X = FString::SanitizeFloat(player->newLocation.X);
		// UEditableText에 값 설정
		Edit_Location_X->SetText(FText::FromString(Location_X));
		Location_Y = FString::SanitizeFloat(player->newLocation.Y);
		Edit_Location_Y->SetText(FText::FromString(Location_Y));
		Location_Z = FString::SanitizeFloat(player->newLocation.Z);
...
{
	Location_X = FString::SanitizeFloat(actorLocation.X);
	Edit_Location_X->SetText(FText::FromString(Location_X));
	Location_Y = FString::SanitizeFloat(actorLocation.Y);
	Edit_Location_Y->SetText(FText::FromString(Location_Y)); 
	Location_Z = FString::SanitizeFloat(actorLocation.Z);
	Edit_Location_Z->SetText(FText::FromString(Location_Z));
}
